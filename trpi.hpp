...
class Trpi{
				...
				if(numObjects<MAX_NUM_OBJECTS){
					for (int index = 0; index >= 0; index = hierarchy[index][0]) {
						...
						if(area>MIN_OBJECT_AREA && area<MAX_OBJECT_AREA && area>refArea){ /*OBJECT FOUND*/
							...
							objectFound = true;
							refArea = area;
								output+=""+intToString((double)(ytmp-y)/10)+","+intToString((areaant-area)/100.00)+",1/"+"\n";
	...
	string seguirGuia(){
		while (1) {
			...
			if (outputObject!=std::string() && outputSymbols!="0.0,0.0,0/\n"){ //GUÍA a no ser que haya una señal STOP
					timeAnt = clock();
					return outputObject;
